	...
	const set: BaileysEventHandler<"messaging-history.set"> = async ({ messages, isLatest }) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during messages set: ${message}`,
			);
		}
	};
				...
				for (const message of messages) {
					try {
						...
						emitEvent(
							...
							undefined,
							"error",
							`An error occured during message upsert: ${message}`,
						);
					}
				}
				break;
		...
		for (const { update, key } of updates) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during message update: ${message}`,
				);
			}
		}
	};
	...
	const del: BaileysEventHandler<"messages.delete"> = async (item) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during message delete: ${message}`,
			);
		}
	};
		...
		for (const { key, receipt } of updates) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during message receipt update: ${message}`,
				);
			}
		}
	};
		...
		for (const { key, reaction } of reactions) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during message reaction update: ${message}`,
				);
			}
		}
	};
